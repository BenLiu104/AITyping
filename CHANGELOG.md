...
- **SenseVoice v2 WS 短效簽名 token（`feat/sensevoice-container-poc`）**：新增後端 `POST /api/sensevoice-token`（回 `{token, expiresAt}`、`Cache-Control: no-store`）。Token 為緊湊、URL-safe、HMAC-SHA256 簽名的 payload（Python stdlib，無 JWT 依賴），payload 含 `v`/`aud`（固定 `sensevoice-ws-v2`）/`exp`/隨機 `nonce`，canonical JSON + unpadded base64url，驗證用 `hmac.compare_digest`。後端 minter（`backend/app/security/sensevoice_token.py`）與 SenseVoice validator（`sensevoice/sensevoice_token.py`）各自實作同一方案、**兩個 Docker context 不互相 import**，靠 source-controlled 固定測試向量 `contracts/sensevoice_ws_token_vectors.json`（兩邊測試套件都 assert）防協定漂移。前端 `SenseVoiceWsClient` 先 POST 取 token，再把 URL-encoded token 以 query parameter 接到 v2 WS URL（瀏覽器無法為 WS 設 header）；取 token 失敗安全回報、不開 socket、token/URL 不 log。config 新增共用 `SENSEVOICE_WS_TOKEN_SECRET` + 有界 `SENSEVOICE_WS_TOKEN_TTL`（預設 60s、界 5–300）。**僅 feature branch，未部署**；生產 SenseVoice 仍 host systemd。

### Changed
- SenseVoice 收斂為 v2 streaming-only runtime：移除未 token-gate 的 `POST /transcribe`、`POST /transcribe_batch`、v1 `/ws/transcribe` 及 full-file FunASR/PyTorch path；Docker image 只保留 ModelScope-pinned ONNX SenseVoiceSmall + FSMN-VAD。新增 v2-only API surface / runtime regression tests，legacy caller 會得到 `404`。
- SenseVoice 容器 POC 修復（`feat/sensevoice-container-poc`，跟進 Attempt-1 review）：(1) **Dockerfile build-cache 重排** — 只將 `requirements.txt` / `fetch_models.py` / `models.sha256` / `funasr_models.sha256` / `verify_funasr_cache.py` / `model_pins.py` 於昂貴 model layer 前 COPY，`api.py` / `tests/` 改為所有 ONNX fetch + FunASR preload + 完整性 verify layer **之後**才 COPY；改 `api.py` 不再 bust 多分鐘 model 層（cache-proof：改動後 ONNX fetch / FunASR preload / verify 三層皆 CACHED，只 COPY api.py 重跑）。(2) **確定性 FunASR model pin** — 新增 `sensevoice/model_pins.py` 為 runtime(`api.py`) 與 build(Dockerfile preload) 共用單一 pin 來源，`api.py` 兩處 `AutoModel` 加 `model_revision=`（SenseVoiceSmall `3847d57b…` / fsmn-vad `df20e6b3…`，取自現行 host cache）。(3) **baked FunASR 完整性** — 新增 `sensevoice/funasr_models.sha256`（9 artifacts）＋ stdlib `sensevoice/verify_funasr_cache.py`，build 內 fail-hard 核對 baked HF cache；streaming ONNX 仍由 package 目錄 `models.sha256` 核對。(4) `modelscope==1.38.1` pin 入 `requirements.txt`，移除 Dockerfile 內獨立 unpinned `pip install modelscope`。(5) `sensevoice/.dockerignore` 補排除 loose model blobs（`*.onnx` / `*.pt` / `*.mvn` / `*.model` 等 + `hub/` / `.cache/` / `modelscope/` / `funasr/`）。(6) 新增 stdlib 靜態 config 契約測試 `sensevoice/tests/test_container_config.py`（10 tests：Dockerfile cache 分層、compose profile gating、model pin 契約）。(7) `DEPLOY.md` 修正重複 heading 編號（§5 測試 → §6 容器化 → §7 已知坑）並補「模型 pin 與完整性（容器）」表。host systemd `sensevoice-api.service`（port 8082）維持 canonical production 邊界，不受影響。
...
### Security
- 移除全部未 token-gate 的 SenseVoice legacy endpoint；`/ws/transcribe-v2` 成為唯一 STT public surface，所有 STT session 先過 backend 簽發的短效 HMAC token。token endpoint 的單人 scope（無 user auth / rate limit）維持不變，擴大公開使用前仍要重新設計。
...
- **SenseVoice v2 WS token gate**：`/ws/transcribe-v2` 現要求後端簽發的短效 HMAC token（query parameter），並在 **log「connection opened」、建立 `StreamingTranscriptionBridge`、載入任何模型之前** 驗證。無效 / 過期 / 錯 audience / secret 缺失一律 generic error + close 1008，不回顯 token。此為 public HF CPU Space 的前置門檻。**⚠️ 已知邊界：** 此 task 只保護 v2 WS；legacy endpoint（`/transcribe`、`/transcribe_batch`、`/ws/transcribe`）未 gate，故單靠此 token 尚不足以安全公開整個 Space——公開前必須另行處理或關閉 legacy endpoint。
