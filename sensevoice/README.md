...
> 廣東話語音轉文字，基於 ModelScope-pinned ONNX SenseVoiceSmall + `sense-voice-streaming-asr` streaming wrapper。
...
|---|---|---|
| GET  | `/ping` | 健康檢查 |
| WS   | `/ws/transcribe-v2` | **唯一 STT 接口；token-gated streaming v2** |
...
**已接受邊界（Ben，2026-07-14）：** `/ws/transcribe-v2` 是唯一 STT 接口，所有 STT session 都必須帶 backend 簽發的短效 HMAC token。token endpoint 仍無 user identity / rate limit；如擴大公開使用、出現 abuse / 成本或加入多人帳戶，必須重新設計 access policy 與 rate limit。
